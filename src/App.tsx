...
type NavKey = "home" | "search_input" | "library" | "history";
...
type DownloadInfo = { songId: string; path: string; bytes: number; totalBytes?: number | null; state: "downloading" | "completed" | "failed" | "cancelled" | string; error?: string | null; lyricsCached: boolean };
...
function App() {
  ...
  const [libraryMode, setLibraryMode] = useState<"mix" | "local" | "songs" | "liked" | "uploaded" | "downloads" | "top" | "playlists" | "albums" | "artists" | "podcasts">("mix");
  ...
  const loadSpotifyProfile = async () => {
    if (!spotifyStatus.authenticated) { setSpotifyProfile(null); return; }
  ...
  const loadLibrary = async (mode: "mix" | "local" | "songs" | "liked" | "uploaded" | "downloads" | "top" | "albums" | "artists" = "mix") => {
    ...
    try {
      ...
      if (mode === "mix") {
        ...
        setLibraryMixSongs([]);
        const command = mode === "local" ? "library_local_files" : mode === "songs" ? "library_songs" : mode === "liked" ? "library_liked_songs" : mode === "uploaded" ? "library_uploaded_songs" : mode === "downloads" ? "library_downloads" : mode === "albums" ? "library_albums" : mode === "artists" ? "library_artists" : null;
  ...
  const loadPodcastItems = async (filter: "episodes" | "channels" | "downloaded") => {
    if (filter === "downloaded") { setLibrary((state) => ({ ...state, status: "loading", error: undefined })); try { setLibrary({ status: "ready", data: await invoke<YtItem[]>("library_downloads") }); } catch (error) { setLibrary({ status: "error", data: [], error: errorMessage(error) }); } return; }
    setLibrary((state) => ({ ...state, status: "loading", error: undefined }));
    try {
      const command = filter === "episodes" ? "ytm_podcast_episodes" : "ytm_podcast_channels";
  ...
  useEffect(() => {
    if (active === "library") {
      ...
      else if (libraryMode === "playlists") void syncSavedPlaylists();
      else if (libraryMode === "albums" || libraryMode === "artists") void loadLibrary(libraryMode);
  ...
  }, [active, historySource, libraryMode, podcastFilter, topPeriod, sessionStatus.authenticated, settings.ytmSync]);
  useEffect(() => {
  ...
  const shareItem = async (item: YtItem) => {
    const url = item.videoId ? `https://music.youtube.com/watch?v=${encodeURIComponent(item.videoId)}` : `https://music.youtube.com/${item.kind}/${encodeURIComponent(item.id)}`;
    try {
  ...
  const performMenuAction = async (action: "open" | "play" | "share" | "copy_link" | "download" | "download_cancel" | "download_remove" | "album" | "episode_save" | "podcast_save" | "queue" | "play_next" | "radio" | "playlist" | "remove_from_playlist" | "remove_history" | "pin" | "unpin" | "artist" | "info" | "edit" | "refetch" | "delete_uploaded" | "change_youtube_version" | "meld_like" | "add_library" | "remove_library", item: YtItem) => {
    ...
    if (action === "podcast_save") {
      if (item.kind !== "episode" || !item.albumId) { setNotice("This episode has no source podcast ID for subscription."); return; }
      const saved = menuState.podcastSaved !== true;
      try {
        await invoke("ytm_toggle_podcast_saved", { podcastId: item.albumId, saved, title: item.albumTitle || "Podcast", author: item.subtitle || null, thumbnail: item.thumbnail ?? null });
        setMenuState((current) => ({ ...current, podcastSaved: saved }));
        setNotice(saved ? `Subscribed to “${item.albumTitle || "podcast"}”.` : `Unsubscribed from “${item.albumTitle || "podcast"}”.`);
      } catch (error) { setNotice(`Podcast subscription update failed: ${errorMessage(error)}`); }
      return;
    }
  ...
  useEffect(() => {
    ...
    audioRef.current.src = player.payload.streamUrl;
    ...
    <div className="app-shell">
      <aside className="sidebar">
        ...
        <nav className="primary-nav" aria-label="Main navigation">
          {navigation.map((item) => <button key={item.key} className={active === item.key ? "nav-item active" : "nav-item"} onClick={() => setActive(item.key)}><span className="nav-icon">{item.icon}</span><span>{item.label}</span></button>)}
                    </nav>
            <nav className="secondary-nav" aria-label="Secondary navigation">
              {secondaryNavigation.map((item) => <button key={item.key} className={active === item.key ? "nav-item active" : "nav-item"} onClick={() => setActive(item.key)}><span className="nav-icon">{item.icon}</span><span>{item.label}</span></button>)}
            </nav>
      ...
      <main className="main-area">
        <header className="topbar">
          <div><p className="eyebrow">Meld Desktop</p><h1>{visibleTitle}</h1></div>
